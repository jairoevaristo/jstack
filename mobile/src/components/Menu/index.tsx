...
export const Menu = ({ onAddToCard, products }: MenuProps) => {
  const [isModalVisible, setIsModalVisible] = useState(false);
  const [selectedProduct, setSelectedProduct] = useState<ProductData | null>(
    null
  );

  const handleOpenModal = useCallback((product: ProductData) => {
  ...
  return (
    <>
      ...
      <FlatList
        ...
        style={{ marginTop: 32 }}
        renderItem={({ item: product }: { item: ProductData }) => (
          <Product onPress={() => handleOpenModal(product)}>
            ...
            <ProductDetail>
              ...
              <Text size={14} color="#6666" style={{ marginVertical: 8 }}>
                {product.description}
              </Text>
            ...
            <AddToCardButton onPress={() => onAddToCard(product)}>
              <PlusCircle />
            </AddToCardButton>
          </Product>
        )}
